...
export const paymentMutations = {
  giveFellowshipOfferingMomo: async (source, args, context) => {
    ...
    try {
      ...
      const stream = memberResponse.records[0]?.get('stream').properties
  ...
  ConfirmTransaction: async (object, args, context) => {
    const session = context.executionContext.session()
    try {
